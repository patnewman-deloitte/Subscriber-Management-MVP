...
export const SegmentsIndexClient = () => {
  ...
  const handleRename = (segment: SegmentDefinition) => {
    ...
    setSegments((prev) =>
      prev.map((item) =>
        item.id === segment.id
          ? { ...item, name: nextName, lastEdited: format(new Date(), "yyyy-MM-dd'T'HH:mm:ssxxx") }
          : item,
      ),
    );
        ...
        {segments.map((segment) => (
          <article
            ...
            className={`rounded-xl border px-4 py-4 shadow-sm transition ${
              segment.archived ? "border-slate-200 bg-slate-100" : "border-slate-200 bg-white hover:border-brand"
            }`}
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-wide text-slate-500">
                  {segment.language.toUpperCase()} · {segment.lastEdited}
                </p>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-slate-500">
              <span className="rounded-full bg-slate-100 px-2 py-1">
                {(segment.metrics.impact.propensity * 100).toFixed(1)}% propensity
              </span>
              <span className="rounded-full bg-slate-100 px-2 py-1">
                Reach email {segment.metrics.reach.email.toLocaleString()}
              </span>
              <span className="rounded-full bg-slate-100 px-2 py-1">
                WhatsApp {segment.metrics.reach.whatsapp.toLocaleString()}
              </span>
            </div>
